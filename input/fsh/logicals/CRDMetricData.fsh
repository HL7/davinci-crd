...
* response          0..* BackboneElement "Returned card/system action"         "Summary information about each card or systemAction returned."
  * type            0..1 code            "Type of CRD response"                "The CRD code for the card/systemAction.  Mandatory if the response corresponds to a type defined by CRD, otherwise omit."
  * type            from CRDCardType             (required)
  * focus           0..* code            "Resource tied to response"           "Indicates the type of resource(s) tied to the card or systemAction."
  * engagement      0..1 code            "ignore | accept | override"          "Indicates if the user engaged with the card as per the CDS Hooks feedback mechanism.  (Not relevant for systemActions.)"
  * coverageInfo    0..* BackboneElement "Coverage information"                "If the response includes a coverage-information systemAction, indicates additional information about the information provided in the coverage-information extension."
    ...
    * questionnaire 0..* BackboneElement "Questionnaire(s) returned"           "Information about the Questionnaire(s) returned to gather additional information (e.g. through DTR)."
      ...
      * response    1..1 boolean         "Pre-pop response provided?"          "If true, it indicates that the response included a partially populated QuestionnaireResponse with answers filled by the payer from pre-known data."
