...
<div class="modified-content" markdown="1">§found-22^crd-client,crd-server^exchange:For this release of the IG, conformant CRD clients and servers **SHALL** support the CDS Hooks [prefetch]({{site.data.fhir.ver.cdshooks}}/index.html#prefetch-template) capability.§  §found-23^crd-client^exchange:Clients **SHALL** be able to supply all the existing resources defined in the [prefetch](#prefetch) section below for request resources they support.§  §found-24?^crd-server^exchange:Servers **SHALL** use prefetch expressions in the manner described if those data elements are relevant to their coverage determination or other decision support.§  
