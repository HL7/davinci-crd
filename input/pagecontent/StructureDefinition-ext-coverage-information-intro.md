...
* **billingCode** indicates the specific billing (codes) the coverage assertion applies to.  This reflects the fact that the code used for an ordered product or service could vary from the performed or billed code.  This is especially useful if the ordered code is not already a 'billing' code, but even if the ordered code _is_ a billing code, the performed code might still vary.
