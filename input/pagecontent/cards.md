...
§resp-39^crd-server^exchange:The CRD server **SHOULD** either prompt for the additional needed information using DTR or return a coverage-information extension indicating that the patient is not covered with a reason indicating the issue (e.g. the member could not be found/resolved).§
...
§§resp-52^crd-server^exchange:When using the "Identify Additional Orders" response type, the proposed orders and any associated resources **SHALL** comply with the required profiles^
...
<div markdown="1" class="modified-content fhir-conformance" id="resp-53^crd-server^exchange" summary="The Request Form Completion response type **SHALL NOT** be used in place of DTR"><a name="FHIR-52588"> </a>
...
§resp-54^crd-server^exchange:Instead of using a card for "Request Form Completion", CRD servers **MAY** opt to use a [systemAction]({{site.data.fhir.ver.cdshooks}}/index.html#system-action).§ §resp-55?^crd-client^exchange:CRD clients supporting the Request Form Completion response type **SHALL** support both the card and systemAction approach.§

§§resp-56^crd-server^exchange:When using the "Request Form Completion" response type, the resources in the card or system action **SHALL** comply with the required profiles^
...
* §resp-58^crd-client^storage:CRD clients **SHOULD** retain a copy of all completed forms for future reference.§
...
§resp-59^crd-server^exchange:Instead of using a card for "Update Coverage Records", CRD servers **MAY** opt to use a [systemAction]({{site.data.fhir.ver.cdshooks}}/index.html#system-action) instead.§ §resp-60^crd-client^exchange:CRD clients supporting the Update Coverage Records response type **SHALL** support both the card and system action approach.§ §resp-61?^crd-client^processing:If receiving a system action, a CRD client **MAY** opt to place the updated record in a holding area for human review rather than directly modifying their source of truth.§

NOTE: This functionality is somewhat redundant with the capabilities of the X12 270/271 transactions. §resp-62^crd-server^exchange:This Update Coverage Records capability **SHALL NOT** be used in situations where regulation dictates the use of the X12 functionality.§
