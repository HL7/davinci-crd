...
In the event decisions are made based on dirty cached data, the unique identifier provided with the Coverage Information extension will allow the payer to trace what information the decision was based on. In general, if a decision is based on information outside the payer's control (e.g., a policy being cancelled), they will not be held to the decision conveyed to the CRD client. If a decision is based on changes within the payer's control (e.g., rules have changed for when prior authorization is required), payers are expected to respect the decision that was conveyed to the CRD client.
