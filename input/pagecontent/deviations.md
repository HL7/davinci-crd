...
<span class="modified-content" markdown="1"><a name="FHIR-52533"> </a>For example, this [CDS Hooks Suggestion]({{site.data.fhir.ver.cdshooks}}/index.html#suggestion) adhering to the [Request Form Completion Response Type](cards.html#request-form-completion-response-type) contains two [Actions]({{site.data.fhir.ver.cdshooks}}/index.html#action) - one referencing an HL7 [Questionnaire]({{site.data.fhir.path}}questionnaire.html) and the other the [Task]({{site.data.fhir.path}}task.html) to complete the Questionnaire.</span>  The Questionnaire will only be created if it didn't already exist:
